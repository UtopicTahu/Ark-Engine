...
qboolean gl_swap_shadows = false; //johnfitz
...
static cvar_t	vid_shadows = { "r_shadows", "0", CVAR_ARCHIVE };
...
static qboolean VID_GetShadows(void)
{
#if defined(USE_SDL2)
	return SDL_GL_GetSwapShadows() == 1;
#else
	int swap_shadows;
	if (SDL_GL_GetAttribute_SHADOWS(SDL_GL_SWAP_CONTROL_SHADOWS, &swap_shadows) == 0)
		return swap_shadows > 0;
	return false;
#endif
}
*\
...
static qboolean VID_SetMode (int width, int height, int refreshrate, int bpp, qboolean fullscreen)
{
	...
	/*
	gl_swap_shadows = true;
	if (SDL_GL_SetSwapShadows((vid_shadows.value) ? 1 : 0) == -1)
		gl_swap_shadows = false;
		*/
	...
	gl_swap_shadows = true;
	if (SDL_GL_SetAttribute_SHADOWS(SDL_GL_SWAP_SHADOWS, (vid_shadows.value) ? 1 : 0) == -1)
		gl_swap_shadows = false;
...
void VID_SyncCvars (void)
{
	if (draw_context)
	{
		...
		//Cvar_SetQuick (&vid_shadows, VID_GetShadows() ? "1" : "0");
	}
...
static void VID_MenuKey (int key)
{
	...
	case K_LEFTARROW:
		...
		case VID_OPT_SHADOWS:
			Cbuf_AddText("toggle vid_shadows\n"); // Utopic Tahu
			break;
		default:
			break;
		}
...
static void VID_MenuDraw (void)
{
	...
	for (i = 0; i < VIDEO_OPTIONS_ITEMS; i++)
	{
		...
		case VID_OPT_SHADOWS:
			...
			if (gl_swap_shadows)
				M_DrawCheckbox(184, y, (int)vid_shadows.value);
			else
				M_Print(184, y, "N/A");
			break;
		case VID_OPT_TEST:
